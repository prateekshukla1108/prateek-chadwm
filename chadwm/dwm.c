...
static void killallinstances(const Arg *arg);
...
cyclelayout(const Arg *arg) {
    ...
    // Count total number of layouts
    for (i = 0; layouts[i].symbol && layouts[i].arrange; i++);
...
void
killallinstances(const Arg *arg)
{
    Client *sel_client = selmon->sel; // Get the currently focused client
    if (!sel_client) return; // Do nothing if no client is selected

    XClassHint ch = { NULL, NULL };

    // Get the class and instance for the selected window
    if (XGetClassHint(dpy, sel_client->win, &ch) == 0)
        return; // Unable to get class hint

    // Iterate over all monitors
    for (Monitor *m = mons; m; m = m->next) {
        // Iterate over all clients in the monitor
        for (Client *c = m->clients; c; c = c->next) {
            XClassHint ch_c = { NULL, NULL };
            if (XGetClassHint(dpy, c->win, &ch_c) != 0) {
                // Compare both class and instance
                if (ch.res_class && ch_c.res_class && strcmp(ch.res_class, ch_c.res_class) == 0 &&
                    ch.res_name && ch_c.res_name && strcmp(ch.res_name, ch_c.res_name) == 0) {
                    XKillClient(dpy, c->win); // Kill the window
                }
            }

            if (ch_c.res_class)
                XFree(ch_c.res_class);
            if (ch_c.res_name)
                XFree(ch_c.res_name);
        }
    }
...
void movetoworkspace(const Arg *arg) {
    ...
    // Get the selected window (client)
    if (!(c = selmon->sel))
        return;
    
    // Get the current tag (first set bit in current tagset)
    curtag = ffs(c->tags) - 1;
    
    // Calculate the new tag based on the argument and current tag
    newtag = curtag + arg->i;
    
    // Ensure the new tag is within range (wrap around between 0 and 6)
    if (newtag < 0)
        newtag = 6;
    else if (newtag > 6)
        newtag = 0;
    
    // Move the selected window to the new tag
    c->tags = 1 << newtag;
    
    // View the new tag
    const Arg viewarg = {.ui = 1 << newtag};
    view(&viewarg);
